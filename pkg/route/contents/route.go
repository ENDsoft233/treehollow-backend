...
func ServicesApiListenHttp() {
	...
	r.GET("/v3/contents/post/randomlist",
		limiterMiddleware(randomListLimiter, "你今天刷了太多鼠洞了，明天再来吧", logger.WARN),
		wanderListPost)
	r.GET("/v3/contents/post/detail",
		limiterMiddleware(detailPostLimiter, "你今天刷了太多鼠洞了，明天再来吧", logger.WARN),
		detailPost)
	r.GET("/v3/contents/search",
		checkParameterPage(consts.SearchMaxPage),
		limiterMiddleware(searchShortTimeLimiter, "请不要短时间内连续搜索鼠洞", logger.INFO),
		limiterMiddleware(searchLimiter, "你今天搜索太多鼠洞了，明天再来吧", logger.WARN),
		searchHotPosts(),
		adminHelpCommand(),
		...
		sysLoadWarningMiddleware(viper.GetFloat64("sys_load_threshold"), "目前鼠洞服务器负载较高，搜索功能已被暂时停用"),
		searchPost)
	...
	r.POST("/v3/send/post",
		auth.DisallowUnregisteredUsers(),
		limiterMiddleware(postLimiter, "请不要短时间内连续发送鼠洞", logger.INFO),
		limiterMiddleware(postLimiter2, "你24小时内已经发送太多鼠洞了", logger.WARN),
	...
	r.POST("/v3/send/comment",
		auth.DisallowUnregisteredUsers(),
		limiterMiddleware(commentLimiter, "请不要短时间内连续发送鼠洞回复", logger.INFO),
		limiterMiddleware(commentLimiter2, "你24小时内已经发送太多鼠洞回复了", logger.WARN),
		disallowBannedPostUsers(),
		checkParameterTextAndImage(),
		sendComment)
	r.POST("/v3/edit/attention",
		auth.DisallowUnregisteredUsers(),
		limiterMiddleware(doAttentionLimiter, "你今天关注太多鼠洞了，明天再来吧", logger.WARN),
		editAttention)
